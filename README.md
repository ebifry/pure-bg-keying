...
主程序是逐帧独立的。做动画序列时有两个地方会露馅，各有一个工具：

**判定要利用时间，不是事后抹平**（`temporal_keying.py`）

单帧的误判一旦进了交集结果，事后对 α 做时域中值只能把痕迹抹平（顺带把边也糊了）。这个工具把时间的利用提到**判定层**：先对两个掩膜序列各自做时域投票（邻居帧多数同意才算前景），再在稳定的判定上取交集。实测同一段 6 帧，帧间抖动 5.10 → 4.00。

**被误扣的像素，回源头查证**（`restore_by_source.py`）

"任何一方说是背景就扣"这条原则会削掉袜子边缘、鞋带、发丝这类细结构。这个工具拿源帧和抠好的帧做比例映射，源帧那里"不是背景色"而这边却是透明的，就恢复回来。**只补不删 —— 它不可能把结果改坏**，最坏情况是没补到东西。

两步的实测数字、限制、以及一个"trimap 未知带一定要薄"的性能教训，都在 [docs/POSTPROCESS.md](docs/POSTPROCESS.md)。
...
temporal_keying.py    ⭐ 时序投票：先在判定层投票，再取交集（序列专用）
restore_by_source.py  ⭐ 源引导补缺失：被误扣的像素，回源头查证（只补不删）
docs/
  TECHNIQUE.md        为什么取交集，每个参数是哪来的，背景色怎么选
  PITFALLS.md         我试过并且失败的七件事，别再试了
  LINE_KEYING.md      顺着勾线切：为什么边界本来就在那儿，以及它现在还做不到什么
  POSTPROCESS.md      投票判定 / 源引导修复，以及我试过但实测更差的两步
